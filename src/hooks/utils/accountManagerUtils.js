...
export const loginOAuth2 = async (
    ...
    if (!url.pathname) {
        const resource = new URL('https://developers.arcgis.com/rest/users-groups-and-items/root.htm');
        const format = 'https://<webadaptorhost>.<domain>.com/<webadaptorname>/sharing/rest';
        console.warn(
...
export const getOrgThumbnail = ({ session: { portal }, portal: { thumbnail, name }, token, orgImage }) => {
    ...
    orgThumbnail.url =
        thumbnail && portal && token ? `${portal}/portals/self/resources/${thumbnail}?token=${token}` : undefined;
...
const createAccountObject = async ({ dSession, portal, clientId }) => {
    try {
        ...
        throw new Error(`Could not create account object. This could be due to UserSession.getUser(). ${e}`);
    }
};
