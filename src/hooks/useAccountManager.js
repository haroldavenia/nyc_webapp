...
import {
    ...
    getUserThumbnail,
    getOrgThumbnail,
} from './utils/accountManagerUtils';
...
const useAccountManager = (
    ...
    const { accounts, status, active, order } = getAccountManagerStorage(managerName);
    ...
    const addAccount = useCallback(
        (options = null, setActive = true, type = 'OAuth2') => {
            ...
            const { clientId, redirectUri, portalUrl, popup, params } = options ? options || {} : managerOptions || {};
    ...
    const restoreAccount = async (account = null) => {
        ...
        if (appId && portalHostname) {
            ...
            if (redirectUri) {
                ...
                console.error(
                    `Cannot restore account. Missing redirectUri and popup in accountManagerState.status: ${accountManagerState.status}`
                );
            }
        } else {
            console.error(`Cannot restore account. Missing clientId and portalUrl in account portal: ${portal}`);
        }
    };
    ...
    const refreshAccount = async (account = null) => {
        ...
        if (response.success && response.session) {
            ...
            console.error(
                `Cannot refresh session for account. Refresh operation failed: ${response.error}. Attempt to re-authenticate failed due to missing properties in account portal item.`
            );
        }
    };
